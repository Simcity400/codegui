    ...
    Effect.gen(function* () {
      ...
      sideForkRouting.codex.startSession.mockImplementationOnce((input) =>
        original(input).pipe(
          ...
          // Stop persists the adapter's live cursor, so the fixture must expose
          // the fork's cursor through both startSession and listSessions.
          Effect.tap((session) =>
            Effect.sync(() => sideForkRouting.codex.updateSession(childId, () => session)),
          ),
        ),
      );
