 ...
 * fall back to bare `$HOME`, which does not contain Claude's session state.
 */
export const resolveClaudeHomePath = Effect.fn("resolveClaudeHomePath")(function* (
  ...
  const inherited = environment?.CLAUDE_CONFIG_DIR ?? "";
...
export const makeClaudeEnvironment = Effect.fn("makeClaudeEnvironment")(function* (
  ...
  const homePath = config.homePath.trim();
  if (homePath.length === 0 && !resolvedBaseEnv.CLAUDE_CONFIG_DIR?.trim()) return resolvedBaseEnv;
...
export const claudeSignedOutMessage = (input: {
  ...
  const configuration =
    input.configDir !== undefined
      ...
      : "";
  return `Claude could not authenticate. For subscription login, run \`claude auth login\` on this environment's machine${configuration}, then retry in this thread. For API-key authentication, check this instance's configured credentials.`;
};
