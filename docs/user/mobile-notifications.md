...
The personal iPhone build receives notifications directly from your T3 server, using its own Apple push credentials. It works over direct, Tailscale, and T3 Connect connections after registering with the server. Keep the server running while agents work. Open the app while work is active to start a Live Activity; it then updates through Apple while the app is in the background. Each environment has its own card. **Settings → Notifications** reports registration or Apple delivery errors and lets you reconnect.
