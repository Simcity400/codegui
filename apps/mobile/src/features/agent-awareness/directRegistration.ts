...
async function refreshNow() {
  ...
  for (const [environmentId, connection] of connections) {
    ...
    try {
      ...
      registered++;
      // The server ends the card once finished rows expire; ending it earlier
      // would strand a follow-up turn on a card that can no longer update.
      if (!liveEnabled || !result.aggregate) {
        if (activity) {
        ...
        else if (result.aggregate.activeCount > 0) {
