  ...
  return (
    ...
    >
      {onToggle ? (
        ...
        <div className="flex items-center gap-1.5 px-1.5 py-1 text-3xs font-medium uppercase tracking-wider text-info-foreground">
          {heading}
        </div>
      )}
      {open
      ...
      {open ? (
        <div className="flex flex-col gap-1">
          ...
          {families.length > 0 ? (
            <div>
              {workflows.length > 0 ? (
                <div className="px-1.5 pt-1 text-3xs font-medium uppercase tracking-wider text-muted-foreground/70">
                  Direct spawns
                </div>
              ) : null}
              {families.map((node) => (
