...
describe("foldSubagentActivities", () => {
  ...
  it("a replayed start for the settled activation only fills metadata", () => {
    const agents = fold([
      activity("task.completed", {
        taskId: "task-2",
        status: "failed",
    ...
    // The same tool use already settled: this is a replay, not a new run.
...
describe("terminal robustness", () => {
  ...
  it("a replayed start after a terminal task.updated does not reopen the run", () => {
    const agents = fold([
      activity("task.updated", {
        taskId: "t1",
        status: "failed",
