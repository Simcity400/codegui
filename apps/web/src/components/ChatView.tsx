...
export default function ChatView(props: ChatViewProps) {
  ...
  const [composerOverlayHeight, setComposerTimelineInset] = useState(0);
  ...
  const publishComposerOverlayHeight = useCallback(
    (height: number) => {
      ...
      const nextInset = nextHeight;
      if (composerOverlayHeightRef.current !== nextInset) {
        composerOverlayHeightRef.current = nextInset;
